...
static void chassis_init(chassis_move_t *chassis_move_init)
{
  ...
  const static fp32 chassis_yaw_pid[3] = {CHASSIS_FOLLOW_GIMBAL_PID_KP, CHASSIS_FOLLOW_GIMBAL_PID_KI, CHASSIS_FOLLOW_GIMBAL_PID_KD};
	const static fp32 chassis_buffer_pid[3] = {1,0.01,100};
	const static fp32 voltage_pid[3] = {200,4,0};
  ...
  for (i = 0; i < 4; i++)
  {
	...
	PID_init(&chassis_move_init->buffer_pid,PID_POSITION,chassis_buffer_pid,50,5);
  //initialize angle PID
  //初始化角度PID
	PID_init(&chassis_move_init->cap_voltage_pid, PID_POSITION, voltage_pid, 20000, 20000);
  PID_init(&chassis_move_init->chassis_angle_pid, PID_POSITION, chassis_yaw_pid, CHASSIS_FOLLOW_GIMBAL_PID_MAX_OUT, CHASSIS_FOLLOW_GIMBAL_PID_MAX_IOUT);
	
//	chassis_move_init->chassis_angle_pid.proportion_output_filter_coefficient = exp(-50*1E-3);
//	chassis_move_init->chassis_angle_pid.derivative_output_filter_coefficient = exp(-50*1E-3);
...
/**
  ...
  * @retval         none
  */
fp32 supercap_vx_set = 8.0;	//8
fp32 supercap_vy_set = 8.0; //8;
fp32 normal_vx_scale = 40.0;
fp32 normal_vy_scale = 40.0;
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
  ...
  rc_deadband_limit(chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_Y_CHANNEL], vy_channel, CHASSIS_RC_DEADLINE);
	...
	if(!cap_state)
	{
		vx_set_channel = vx_channel /660.0f *((float)max_power_limit/normal_vx_scale);
		vy_set_channel = -vy_channel /660.0f *((float)max_power_limit/normal_vx_scale);
	}
	else
	{
		vx_set_channel = vx_channel /660.0f*(supercap_vx_set);
		vy_set_channel = -vy_channel /660.0f*(supercap_vy_set);
	}
	...
	if(!(chassis_move_rc_to_vector ->chassis_RC ->key.v & KEY_PRESSED_OFFSET_CTRL))
	{
		...
		else
		{
			if(cap_state)
			{
				if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_FRONT_KEY)
				{
					vx_set_channel = supercap_vx_set;
				}
				else if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_BACK_KEY)
				{
					vx_set_channel = -supercap_vx_set;
				...
				if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_LEFT_KEY)
				{
					vy_set_channel = supercap_vy_set;
				}
				else if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_RIGHT_KEY)
				{
					vy_set_channel = -supercap_vy_set;
			...
			else
			{
				if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_FRONT_KEY)
				{
					vx_set_channel = ((float)max_power_limit/normal_vx_scale);
				}
				else if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_BACK_KEY)
				{
					vx_set_channel = -((float)max_power_limit/normal_vx_scale);
				...
				if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_LEFT_KEY)
				{
					vy_set_channel = ((float)max_power_limit/normal_vy_scale);
				}
				else if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_RIGHT_KEY)
				{
					vy_set_channel = -((float)max_power_limit/normal_vy_scale);
...
float wz_scale = 10.0;
float wz_scale_cap = 7.1428571428571428571428571428571;
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
  ...
  if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW)
  {
    ...
    if (chassis_move_control->swing_flag)
    {
      ...
      //set control relative angle  set-point
			if(cap_state)
			{
                //这里的chassis_power是最大功率
				chassis_move_control->wz_set = chassis_power / wz_scale_cap;
			}
			else
			{
				chassis_move_control->wz_set = chassis_power / wz_scale;
			}
      //speed limit
      //速度限幅
...
static void chassis_vector_to_omni_wheel_speed(const fp32 vx_set, const fp32 vy_set, const fp32 wz_set, fp32 wheel_speed[4])
{
  ...
  wheel_speed[0] = (-vx_set*sqrt(2)/2 - vy_set*sqrt(2)/2 + wz_set * 0.4);//0.0763;
  wheel_speed[1] = (vx_set*sqrt(2)/2 - vy_set*sqrt(2)/2 + wz_set * 0.4);//0.0763;
  wheel_speed[2] = (vx_set*sqrt(2)/2 + vy_set*sqrt(2)/2 + wz_set * 0.4);//0.0763;
  wheel_speed[3] = (-vx_set*sqrt(2)/2 + vy_set*sqrt(2)/2 + wz_set * 0.4);//0.0763;
}
